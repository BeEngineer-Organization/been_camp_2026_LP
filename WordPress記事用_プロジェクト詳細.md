# BeEngineer合宿案内サイト - WordPress記事用プロジェクト詳細

## 基本情報

- **テンプレートファイル**: 参考なし（新規作成）
- **新しいファイル名**: `templates/projects/project-been-camp.php`（想定）
...
- **画像フォルダパス**: `assets/` 配下
- **使用する画像枚数**: 3-5枚（推奨）
  - `been_camp_1.png` - トップページ（ヒーローセクション）
  - `been_camp_2.png` - スケジュール・タブ切り替え機能
  - `been_camp_3.png` - モバイル表示（ハンバーガーメニュー）
...
**1. フレームワークを使わない軽量設計**
ReactやVue.jsなどのフレームワークを使用せず、Vanilla JavaScriptで実装することで、バンドルサイズを最小限に抑え、高速なページ読み込みを実現しました。依存関係がなく、メンテナンスが容易な構造となっています。

**2. レスポンシブデザインの徹底**
PC版とSP版で異なるUIパターンを採用しています。PC版では固定サイドバーとタブ切り替え、SP版ではハンバーガーメニューとアコーディオン形式を採用し、それぞれのデバイスに最適化された操作性を実現しました。

**3. ユーザビリティの向上**
localStorageを活用したチェックリストの状態保存、スムーススクロールによる快適なナビゲーション体験、スクロールスパイによる現在位置の可視化など、ユーザーが快適に使える機能を実装しました。

**4. デザインの一貫性**
CSS Custom Propertiesによる統一されたカラーパレット管理により、ロゴスランド風の温かみのあるデザインを実現。パステルカラーを活用した視覚的な階層構造により、情報が整理されて見やすくなっています。

**5. 保守性の高いコード構造**
機能ごとに分割されたJavaScript関数、セマンティックHTMLの使用、コメントによる詳細な説明により、将来のメンテナンスや機能追加が容易なコード構造を実現しました。
...
スクロール位置に応じて、現在表示されているセクションに対応するナビゲーションリンクを自動的にハイライトする機能を実装。パフォーマンスを考慮し、Intersection Observer APIを使用することで、従来のscrollイベントよりも効率的に動作します。
...
サーバーサイドの実装なしで、クライアント側のみでチェックリストの状態を永続化。ページを閉じても状態が保持されるため、ユーザー体験が向上します。
...
CSS変数を使用することで、カラーパレットやデザイントークンの一元管理を実現。テーマ変更が容易で、デザインシステムの保守性が向上します。

### 活用シーン

**1. イベント・合宿の案内サイト**
開催情報の一元管理、参加者への情報提供、スケジュールや持ち物リストの共有など、イベント運営に必要な情報を効率的に提供できます。

**2. 教育機関のイベント告知**
学習プログラムの紹介、会場情報の提供、参加者向けの詳細情報など、教育イベントの告知サイトとして活用できます。

**3. カスタマイズ可能なランディングページ**
テンプレートとして再利用可能で、セクションの追加・削除が容易、デザインのカスタマイズが簡単なため、様々な用途に応用できます。
...
- **特記事項**: 
  - フレームワークを使用しない軽量設計により、高速なページ読み込みを実現
  - 静的サイトとして動作するため、サーバーサイドの設定が不要
  - GitHub Pagesでのホスティングに対応
  - モバイルファーストな設計思想により、スマートフォンでの操作性を重視
